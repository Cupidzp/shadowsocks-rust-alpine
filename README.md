...
先在 GitHub 网页创建一个空仓库，例如 `shadowsocks-rust-alpine`。不要初始化 README、
`.gitignore` 或 License，避免首次推送时产生无关冲突。
...
git remote add origin https://github.com/USERNAME/shadowsocks-rust-alpine.git
git push -u origin main
```

把 `USERNAME` 和仓库名称替换成自己的值。使用 HTTPS 推送时，GitHub 要求使用
Personal Access Token 作为密码；也可以先配置 SSH 远程地址：

```powershell
git remote set-url origin git@github.com:USERNAME/shadowsocks-rust-alpine.git
git push -u origin main
```

推送后，将 README 中的占位地址替换为真实地址：

```text
https://raw.githubusercontent.com/USERNAME/shadowsocks-rust-alpine/main/install.sh
```

然后其他 Alpine 服务器可以执行：

```sh
apk add --no-cache curl && curl -fsSL https://raw.githubusercontent.com/USERNAME/shadowsocks-rust-alpine/main/install.sh | sh
```
...
```sh
curl -fsSLo /root/install-shadowsocks-rust.sh https://raw.githubusercontent.com/USERNAME/shadowsocks-rust-alpine/main/install.sh
